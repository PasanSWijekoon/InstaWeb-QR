...
const logoUrlPrompt = document.getElementById('logo-url-prompt') as HTMLInputElement;
...
const publishResult = document.getElementById('publish-result') as HTMLDivElement;
const repoLink = document.getElementById('repo-link') as HTMLAnchorElement;
const liveSiteLink = document.getElementById('live-site-link') as HTMLAnchorElement;
...
try {
    ...
    generateBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            let prompt = "You are an expert web developer. Create a single-page responsive HTML file with inline CSS based on the content and layout of this image. The HTML should be self-contained in a single file. Do not include any external scripts or stylesheets. Ensure the generated code is clean, modern, and accessible. Extract any text from the image and use it in the HTML.";
            if (logoUrl) {
                prompt += ` If the image contains a logo, use this URL for the logo's src attribute: ${logoUrl}.`;
            }
            prompt += " Respond ONLY with the HTML code inside a single markdown code block.";
            ...
            console.error(error);
            alert('Failed to generate website. Please check the console for details.');
        } finally {
            hideLoader();
    ...
    publishBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (!userResponse.ok) throw new Error(`GitHub authentication failed: ${await userResponse.text()}`);
            ...
            const repoResponse = await fetch('https://api.github.com/user/repos', {
                ...
                body: JSON.stringify({ name: repo, description: 'Website generated by InstaWeb QR' })
            });
            if (repoResponse.status !== 201) throw new Error(`Failed to create repository: ${await repoResponse.text()}`);
            ...
            await fetch(`https://api.github.com/repos/${owner}/${actualRepoName}/contents/index.html`, {
                method: 'PUT',
                headers: {
            ...
            const pagesResponse = await fetch(`https://api.github.com/repos/${owner}/${actualRepoName}/pages`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            if (!pagesResponse.ok) throw new Error(`Failed to enable GitHub Pages: ${await pagesResponse.text()}`);
            ...
            qrCodeImg.src = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(liveUrl)}`;
            ...
            const checkStatus = async () => {
                ...
                try {
                    const response = await fetch(liveUrl, { method: 'HEAD', mode: 'no-cors' });
    ...
    document.body.innerHTML = `<div style="color: red; padding: 20px; font-family: sans-serif;">
        ...
        <pre style="white-space: pre-wrap; word-break: break-all;">${(error as Error).message}</pre>
    </div>`
    console.error(error);
}
